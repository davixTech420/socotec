...
const TablaComponente = ({
  ...
  return (
    <>
                          ...
                          {columns.map((column) => (
                            ...
                            >
                              {column.key === 'usuariosGrupo' ? (
                                <TouchableOpacity style={{ justifyContent: 'center', alignItems: 'center' }} onPress={() => setOpenUserGroup(true)}>
      ...
      <AlertaScroll onOpen={openUserGroup} onClose={() => setOpenUserGroup(false)} title="Usuarios del grupo" content={<Text>asdasd</Text>} actions={[<Button>Cancelar</Button>, <Button>Crear</Button>]} />
    </>
  )
}
    ...
    (item) => {
      if (onEdit) {
        onEdit(item)
          ...
          .catch((error) => {
            setSnackbarMessage({
              text: `Error al actualizar el registro: ${error.message}`,
              type: "error",
            })
            setSnackbarVisible(true)
          })
