...
import { ScrollView, Platform, Dimensions, Modal, View, BackHandler, FlatList, Pressable, ImageBackground } from 'react-native';
...
const ProjectDetailsModal = ({ visible, project, onDismiss }) => {
  const theme = useTheme();
    ...
    >
      <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.95)' }}>
        ...
        <Animated.ScrollView 
          ...
          // Desactivar scroll mientras carga en móviles
        ...
        >
          ...
          >
            ...
            <View style={{ padding: 16 }}>
              {/* Simplificar animaciones en móviles */}
              <View>
                <Text variant="headlineMedium" style={{ fontWeight: 'bold' }}>{project.nombre}</Text>
                <Chip 
                  icon="tag" 
                ...
                <Surface style={{
                  ...
                  <View style={{ gap: 12 }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      <Text variant="bodyMedium" style={{ fontWeight: '500' }}>Área:</Text>
                      <Text variant="bodyMedium">{project.superficie || '1,200 m²'}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      <Text variant="bodyMedium" style={{ fontWeight: '500' }}>Ubicación:</Text>
                      <Text variant="bodyMedium">{project.ubicacion || 'Ciudad de México'}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      <Text variant="bodyMedium" style={{ fontWeight: '500' }}>Año:</Text>
                      <Text variant="bodyMedium">{project.createdAt || '2023'}</Text>
                    </View>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      <Text variant="bodyMedium" style={{ fontWeight: '500' }}>Cliente:</Text>
...
const ProjectCard = ({ project, onPress, index }) => {
  const theme = useTheme();
  ...
  const animatedStyle = useAnimatedStyle(() => ({
    transform: [
      { scale: scale.value },
      { rotate: IS_MOBILE ? '0deg' : `${rotation.value}deg` }
    ],
  }));

  return (
    <Animated.View
      entering={IS_MOBILE ? FadeIn.delay(100) : (index % 2 === 0 ? SlideInLeft.delay(300 + index * 100) : SlideInRight.delay(300 + index * 100))}
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      >
        ...
        {isLoading ? (
          ...
          // Usar FlatList en lugar de map para mejor rendimiento en listas largas
          IS_MOBILE ? (
            <FlatList
