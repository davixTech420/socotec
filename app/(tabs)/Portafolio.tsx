...
import { ScrollView, Platform, Dimensions, Modal, View, BackHandler } from 'react-native';
import { Card, Text, Button, useTheme, Surface, Avatar, IconButton } from 'react-native-paper';
import { SafeAreaView } from 'react-native-safe-area-context';
import { MaterialCommunityIcons } from '@expo/vector-icons';
import Animated, { FadeInUp, FadeInDown, withSpring, useAnimatedStyle, useSharedValue } from 'react-native-reanimated';
...
export default function Portafolio() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      return () => {
        // Clean up any resources when screen loses focus
  ...
  const closeProjectModal = () => {
    setIsModalVisible(false);
    // Small delay before clearing the project data
    setTimeout(() => {
      setSelectedProject(null);
    }, 300);
  };
