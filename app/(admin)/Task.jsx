...
import { createTask, getTask, deleteTask, activeTask, inactiveTask, updateTask, getUsers } from "@/services/adminServices";
...
const Task = () => {
  ...
  useFocusEffect(
    useCallback(() => {
      getTask().then(setData).catch(console.error)
  ...
  const handleSubmit = useCallback(async () => {
    try {
      const requiredFields = isEditing ? ["asignadoId", "titulo", "descripcion"] : ["asignadoId", "titulo", "descripcion"];
    ...
    try {
      ...
      setData((prevData) =>
        prevData.map((dataItem) =>
          dataItem.id === item.id ? { ...dataItem, estado: action === activeTask } : dataItem,
        ),
      )
    } catch (error) {
      console.log(`Error al ${action === activeTask ? "activar" : "desactivar"} la tarea:`, error)
      throw error;
    }
  }, [])
  ...
  return (
    <>
      <PaperProvider theme={theme}>
        ...
        <AlertaScroll onOpen={openForm} onClose={resetForm} title={isEditing ? "Editar Tarea" : "Nueva Tarea"} content={
          <View style={{
